...
import { useTTreatmentDiscountStore } from '@/modules/hospital/t-treatment-discount/store';
import { computed, onMounted, ref } from 'vue';

const store = useTTreatmentDiscountStore();
...
const openDetailModal = (item: any) => {
  ...
  (document.getElementById('t_treatment_discount_detail_modal') as HTMLDialogElement).showModal();
};
...
const openModal = (item: any = null) => {
  ...
  if (item) {
    ...
    store.data = { doctorOfficeTreatmentPriceId: null, value: null };
  }
  (document.getElementById('t_treatment_discount_modal') as HTMLDialogElement).showModal();
};

const submitForm = async () => {
  await store.saveData(selectedId.value);
  (document.getElementById('t_treatment_discount_modal') as HTMLDialogElement).close();
};
...
<template>
  <div class="p-6">
    <div class="flex justify-between items-center mb-4">
      <h1 class="text-2xl font-bold">T Treatment Discount Management</h1>
      ...
      <table class="table table-zebra w-full">
        <thead>
          <tr class="bg-base-200">
            <th>ID</th>
            <th>Doctor Office Treatment Price Id</th>
            <th>Value</th>
        ...
        <tbody>
          ...
          <tr v-for="data in store.pagedata" :key="data.id" class="hover">
            <th>{{ data.id }}</th>
            <td class="font-medium">{{ data.doctorOfficeTreatmentPriceId || '-' }}</td>
            <td class="font-medium">{{ data.value || '-' }}</td>
            <td>
    ...
    <!-- Details View Modal -->
    <dialog id="t_treatment_discount_detail_modal" class="modal">
      <div class="modal-box max-w-md">
        <h3 class="font-bold text-xl mb-4">T Treatment Discount Details</h3>
        ...
        <div v-if="selectedItem" class="flex flex-col items-center space-y-4">
          ...
          <div class="w-full bg-base-200 rounded-box p-4 space-y-3">
            ...
            <div class="flex justify-between border-b border-base-300 pb-2">
              <span class="text-sm font-medium opacity-70">Doctor Office Treatment Price Id</span>
              <span class="font-semibold">{{ selectedItem.doctorOfficeTreatmentPriceId ?? '-' }}</span>
            </div>
            <div class="flex justify-between border-b border-base-300 pb-2">
              <span class="text-sm font-medium opacity-70">Value</span>
              <span class="font-semibold">{{ selectedItem.value ?? '-' }}</span>
            </div>
          </div>
        </div>
    ...
    <dialog id="t_treatment_discount_modal" class="modal">
      <div class="modal-box">
        <h3 class="font-bold text-lg">{ isEdit ? 'Edit' : 'Create' } T Treatment Discount</h3>
        
        <div class="py-4 space-y-4">
          <div class="form-control">
            <label class="label"><span class="label-text">Doctor Office Treatment Price Id</span></label>
            <input v-model.number="store.data.doctorOfficeTreatmentPriceId" type="number" class="input input-bordered w-full" />
          </div>

          <div class="form-control">
            <label class="label"><span class="label-text">Value</span></label>
            <input v-model.number="store.data.value" type="number" class="input input-bordered w-full" />
          </div>
        </div>
