...
import { useMCustomerMemberStore } from '@/modules/hospital/m-customer-member/store';
import { computed, onMounted, ref } from 'vue';

const store = useMCustomerMemberStore();
...
const openDetailModal = (item: any) => {
  ...
  (document.getElementById('m_customer_member_detail_modal') as HTMLDialogElement).showModal();
};
...
const openModal = (item: any = null) => {
  ...
  if (item) {
    ...
    store.data = { parentBiodataId: null, customerId: null, customerRelationId: null };
  }
  (document.getElementById('m_customer_member_modal') as HTMLDialogElement).showModal();
};

const submitForm = async () => {
  await store.saveData(selectedId.value);
  (document.getElementById('m_customer_member_modal') as HTMLDialogElement).close();
};
...
<template>
  <div class="p-6">
    <div class="flex justify-between items-center mb-4">
      <h1 class="text-2xl font-bold">M Customer Member Management</h1>
      ...
      <table class="table table-zebra w-full">
        <thead>
          <tr class="bg-base-200">
            <th>ID</th>
            <th>Parent Biodata Id</th>
            <th>Customer Id</th>
            <th>Customer Relation Id</th>
        ...
        <tbody>
          ...
          <tr v-for="data in store.pagedata" :key="data.id" class="hover">
            <th>{{ data.id }}</th>
            <td class="font-medium">{{ data.parentBiodataId || '-' }}</td>
            <td class="font-medium">{{ data.customerId || '-' }}</td>
            <td class="font-medium">{{ data.customerRelationId || '-' }}</td>
            <td>
    ...
    <!-- Details View Modal -->
    <dialog id="m_customer_member_detail_modal" class="modal">
      <div class="modal-box max-w-md">
        <h3 class="font-bold text-xl mb-4">M Customer Member Details</h3>
        ...
        <div v-if="selectedItem" class="flex flex-col items-center space-y-4">
          ...
          <div class="w-full bg-base-200 rounded-box p-4 space-y-3">
            ...
            <div class="flex justify-between border-b border-base-300 pb-2">
              <span class="text-sm font-medium opacity-70">Parent Biodata Id</span>
              <span class="font-semibold">{{ selectedItem.parentBiodataId ?? '-' }}</span>
            </div>
            <div class="flex justify-between border-b border-base-300 pb-2">
            ...
            <div class="flex justify-between border-b border-base-300 pb-2">
              <span class="text-sm font-medium opacity-70">Customer Relation Id</span>
              <span class="font-semibold">{{ selectedItem.customerRelationId ?? '-' }}</span>
            </div>
          </div>
        </div>
    ...
    <dialog id="m_customer_member_modal" class="modal">
      <div class="modal-box">
        <h3 class="font-bold text-lg">{ isEdit ? 'Edit' : 'Create' } M Customer Member</h3>
        
        <div class="py-4 space-y-4">
          <div class="form-control">
            <label class="label"><span class="label-text">Parent Biodata Id</span></label>
            <input v-model.number="store.data.parentBiodataId" type="number" class="input input-bordered w-full" />
          </div>
          ...
          <div class="form-control">
            <label class="label"><span class="label-text">Customer Relation Id</span></label>
            <input v-model.number="store.data.customerRelationId" type="number" class="input input-bordered w-full" />
          </div>
        </div>
