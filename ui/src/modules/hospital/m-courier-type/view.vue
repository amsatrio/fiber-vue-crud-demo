...
import { useMCourierTypeStore } from '@/modules/hospital/m-courier-type/store';
import { computed, onMounted, ref } from 'vue';

const store = useMCourierTypeStore();
...
const openDetailModal = (item: any) => {
  ...
  (document.getElementById('m_courier_type_detail_modal') as HTMLDialogElement).showModal();
};
...
const openModal = (item: any = null) => {
  ...
  if (item) {
    ...
    store.data = { courierId: null, name: '' };
  }
  (document.getElementById('m_courier_type_modal') as HTMLDialogElement).showModal();
};

const submitForm = async () => {
  await store.saveData(selectedId.value);
  (document.getElementById('m_courier_type_modal') as HTMLDialogElement).close();
};
...
<template>
  <div class="p-6">
    <div class="flex justify-between items-center mb-4">
      <h1 class="text-2xl font-bold">M Courier Type Management</h1>
      ...
      <table class="table table-zebra w-full">
        ...
        <tbody>
          ...
          <tr v-for="data in store.pagedata" :key="data.id" class="hover">
            <th>{{ data.id }}</th>
            <td class="font-medium">{{ data.courierId || '-' }}</td>
    ...
    <!-- Details View Modal -->
    <dialog id="m_courier_type_detail_modal" class="modal">
      <div class="modal-box max-w-md">
        <h3 class="font-bold text-xl mb-4">M Courier Type Details</h3>
        ...
        <div v-if="selectedItem" class="flex flex-col items-center space-y-4">
          ...
          <div class="w-full bg-base-200 rounded-box p-4 space-y-3">
            ...
            <div class="flex justify-between border-b border-base-300 pb-2">
              <span class="text-sm font-medium opacity-70">Courier Id</span>
              <span class="font-semibold">{{ selectedItem.courierId ?? '-' }}</span>
            </div>
            <div class="flex justify-between border-b border-base-300 pb-2">
    ...
    <dialog id="m_courier_type_modal" class="modal">
      <div class="modal-box">
        <h3 class="font-bold text-lg">{{ isEdit ? 'Edit' : 'Create' }} M Courier Type</h3>
        
        <div class="py-4 space-y-4">
          <div class="form-control">
            <label class="label"><span class="label-text">Courier Id</span></label>
            <input v-model.number="store.data.courierId" type="number" class="input input-bordered w-full" />
          </div>
