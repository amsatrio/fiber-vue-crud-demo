...
import { useMBloodGroupStore } from '@/modules/hospital/m-blood-group/store';
import { computed, onMounted, ref } from 'vue';

const store = useMBloodGroupStore();
...
const openDetailModal = (item: any) => {
  ...
  (document.getElementById('m_blood_group_detail_modal') as HTMLDialogElement).showModal();
};
...
const openModal = (item: any = null) => {
  ...
  if (item) {
    ...
    store.data = { code: '', descrtiption: '' };
  }
  (document.getElementById('m_blood_group_modal') as HTMLDialogElement).showModal();
};

const submitForm = async () => {
  await store.saveData(selectedId.value);
  (document.getElementById('m_blood_group_modal') as HTMLDialogElement).close();
};
...
<template>
  <div class="p-6">
    <div class="flex justify-between items-center mb-4">
      <h1 class="text-2xl font-bold">M Blood Group Management</h1>
      ...
      <table class="table table-zebra w-full">
        ...
        <tbody>
          ...
          <tr v-for="data in store.pagedata" :key="data.id" class="hover">
            ...
            <td class="font-medium">{{ data.descrtiption || '-' }}</td>
            <td>
    ...
    <!-- Details View Modal -->
    <dialog id="m_blood_group_detail_modal" class="modal">
      <div class="modal-box max-w-md">
        <h3 class="font-bold text-xl mb-4">M Blood Group Details</h3>
        ...
        <div v-if="selectedItem" class="flex flex-col items-center space-y-4">
          ...
          <div class="w-full bg-base-200 rounded-box p-4 space-y-3">
            ...
            <div class="flex justify-between border-b border-base-300 pb-2">
              <span class="text-sm font-medium opacity-70">Code</span>
              <span class="font-semibold">{{ selectedItem.code ?? '-' }}</span>
            </div>
            <div class="flex justify-between border-b border-base-300 pb-2">
              <span class="text-sm font-medium opacity-70">Descrtiption</span>
              <span class="font-semibold">{{ selectedItem.descrtiption ?? '-' }}</span>
            </div>
          </div>
        </div>
    ...
    <dialog id="m_blood_group_modal" class="modal">
      <div class="modal-box">
        <h3 class="font-bold text-lg">{{ isEdit ? 'Edit' : 'Create' }} M Blood Group</h3>
        
        <div class="py-4 space-y-4">
          <div class="form-control">
            <label class="label"><span class="label-text">Code</span></label>
            <input v-model="store.data.code" type="text" class="input input-bordered w-full" />
          </div>

          <div class="form-control">
            <label class="label"><span class="label-text">Descrtiption</span></label>
            <input v-model="store.data.descrtiption" type="text" class="input input-bordered w-full" />
          </div>
        </div>
