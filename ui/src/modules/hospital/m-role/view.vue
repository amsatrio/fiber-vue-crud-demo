...
import { useMRoleStore } from '@/modules/hospital/m-role/store';
import { computed, onMounted, ref } from 'vue';

const store = useMRoleStore();
...
const openDetailModal = (item: any) => {
  ...
  (document.getElementById('m_role_detail_modal') as HTMLDialogElement).showModal();
};
...
const openModal = (item: any = null) => {
  ...
  if (item) {
    ...
    store.data = { name: '', code: '' };
  }
  (document.getElementById('m_role_modal') as HTMLDialogElement).showModal();
};

const submitForm = async () => {
  await store.saveData(selectedId.value);
  (document.getElementById('m_role_modal') as HTMLDialogElement).close();
};
...
<template>
  <div class="p-6">
    <div class="flex justify-between items-center mb-4">
      <h1 class="text-2xl font-bold">M Role Management</h1>
    ...
    <!-- Details View Modal -->
    <dialog id="m_role_detail_modal" class="modal">
      <div class="modal-box max-w-md">
        <h3 class="font-bold text-xl mb-4">M Role Details</h3>
    ...
    <dialog id="m_role_modal" class="modal">
      <div class="modal-box">
        <h3 class="font-bold text-lg">{ isEdit ? 'Edit' : 'Create' } M Role</h3>
