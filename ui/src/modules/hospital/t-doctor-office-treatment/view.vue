...
import { useTDoctorOfficeTreatmentStore } from '@/modules/hospital/t-doctor-office-treatment/store';
import { computed, onMounted, ref } from 'vue';

const store = useTDoctorOfficeTreatmentStore();
...
const openDetailModal = (item: any) => {
  ...
  (document.getElementById('t_doctor_office_treatment_detail_modal') as HTMLDialogElement).showModal();
};
...
const openModal = (item: any = null) => {
  ...
  if (item) {
    ...
    store.data = { doctorTreatmentId: null, doctorOfficeId: null };
  }
  (document.getElementById('t_doctor_office_treatment_modal') as HTMLDialogElement).showModal();
};

const submitForm = async () => {
  await store.saveData(selectedId.value);
  (document.getElementById('t_doctor_office_treatment_modal') as HTMLDialogElement).close();
};
...
<template>
  <div class="p-6">
    <div class="flex justify-between items-center mb-4">
      <h1 class="text-2xl font-bold">T Doctor Office Treatment Management</h1>
      ...
      <table class="table table-zebra w-full">
        ...
        <tbody>
          ...
          <tr v-for="data in store.pagedata" :key="data.id" class="hover">
            <th>{{ data.id }}</th>
            <td class="font-medium">{{ data.doctorTreatmentId || '-' }}</td>
    ...
    <!-- Details View Modal -->
    <dialog id="t_doctor_office_treatment_detail_modal" class="modal">
      <div class="modal-box max-w-md">
        <h3 class="font-bold text-xl mb-4">T Doctor Office Treatment Details</h3>
        ...
        <div v-if="selectedItem" class="flex flex-col items-center space-y-4">
          ...
          <div class="w-full bg-base-200 rounded-box p-4 space-y-3">
            ...
            <div class="flex justify-between border-b border-base-300 pb-2">
              <span class="text-sm font-medium opacity-70">Doctor Treatment Id</span>
              <span class="font-semibold">{{ selectedItem.doctorTreatmentId ?? '-' }}</span>
            </div>
            <div class="flex justify-between border-b border-base-300 pb-2">
    ...
    <dialog id="t_doctor_office_treatment_modal" class="modal">
      <div class="modal-box">
        <h3 class="font-bold text-lg">{ isEdit ? 'Edit' : 'Create' } T Doctor Office Treatment</h3>
        
        <div class="py-4 space-y-4">
          <div class="form-control">
            <label class="label"><span class="label-text">Doctor Treatment Id</span></label>
            <input v-model.number="store.data.doctorTreatmentId" type="number" class="input input-bordered w-full" />
          </div>
