...
import { useTResetPasswordStore } from '@/modules/hospital/t-reset-password/store';
import { computed, onMounted, ref } from 'vue';

const store = useTResetPasswordStore();
...
const openDetailModal = (item: any) => {
  ...
  (document.getElementById('t_reset_password_detail_modal') as HTMLDialogElement).showModal();
};
...
const openModal = (item: any = null) => {
  ...
  if (item) {
    ...
    store.data = { oldPassword: '', newPassword: '', resetFor: '' };
  }
  (document.getElementById('t_reset_password_modal') as HTMLDialogElement).showModal();
};

const submitForm = async () => {
  await store.saveData(selectedId.value);
  (document.getElementById('t_reset_password_modal') as HTMLDialogElement).close();
};
...
<template>
  <div class="p-6">
    <div class="flex justify-between items-center mb-4">
      <h1 class="text-2xl font-bold">T Reset Password Management</h1>
      ...
      <table class="table table-zebra w-full">
        ...
        <tbody>
          ...
          <tr v-for="data in store.pagedata" :key="data.id" class="hover">
            <th>{{ data.id }}</th>
            <td class="font-medium">{{ data.oldPassword || '-' }}</td>
            <td class="font-medium">{{ data.newPassword || '-' }}</td>
            <td class="font-medium">{{ data.resetFor || '-' }}</td>
            <td>
    ...
    <!-- Details View Modal -->
    <dialog id="t_reset_password_detail_modal" class="modal">
      <div class="modal-box max-w-md">
        <h3 class="font-bold text-xl mb-4">T Reset Password Details</h3>
        ...
        <div v-if="selectedItem" class="flex flex-col items-center space-y-4">
          ...
          <div class="w-full bg-base-200 rounded-box p-4 space-y-3">
            ...
            <div class="flex justify-between border-b border-base-300 pb-2">
              <span class="text-sm font-medium opacity-70">Old Password</span>
              <span class="font-semibold">{{ selectedItem.oldPassword ?? '-' }}</span>
            </div>
            <div class="flex justify-between border-b border-base-300 pb-2">
              <span class="text-sm font-medium opacity-70">New Password</span>
              <span class="font-semibold">{{ selectedItem.newPassword ?? '-' }}</span>
            </div>
            <div class="flex justify-between border-b border-base-300 pb-2">
              <span class="text-sm font-medium opacity-70">Reset For</span>
              <span class="font-semibold">{{ selectedItem.resetFor ?? '-' }}</span>
            </div>
          </div>
        </div>
    ...
    <dialog id="t_reset_password_modal" class="modal">
      <div class="modal-box">
        <h3 class="font-bold text-lg">{ isEdit ? 'Edit' : 'Create' } T Reset Password</h3>
        
        <div class="py-4 space-y-4">
          <div class="form-control">
            <label class="label"><span class="label-text">Old Password</span></label>
            <input v-model="store.data.oldPassword" type="text" class="input input-bordered w-full" />
          </div>

          <div class="form-control">
            <label class="label"><span class="label-text">New Password</span></label>
            <input v-model="store.data.newPassword" type="text" class="input input-bordered w-full" />
          </div>

          <div class="form-control">
            <label class="label"><span class="label-text">Reset For</span></label>
            <input v-model="store.data.resetFor" type="text" class="input input-bordered w-full" />
          </div>
        </div>
