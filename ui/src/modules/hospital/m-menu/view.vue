...
import { useMMenuStore } from '@/modules/hospital/m-menu/store';
import { computed, onMounted, ref } from 'vue';

const store = useMMenuStore();
...
const openDetailModal = (item: any) => {
  ...
  (document.getElementById('m_menu_detail_modal') as HTMLDialogElement).showModal();
};
...
const openModal = (item: any = null) => {
  ...
  if (item) {
    ...
    store.data = { name: '', url: '', parentId: null, bigIcon: '', smallIcon: '' };
  }
  (document.getElementById('m_menu_modal') as HTMLDialogElement).showModal();
};

const submitForm = async () => {
  await store.saveData(selectedId.value);
  (document.getElementById('m_menu_modal') as HTMLDialogElement).close();
};
...
<template>
  <div class="p-6">
    <div class="flex justify-between items-center mb-4">
      <h1 class="text-2xl font-bold">M Menu Management</h1>
      ...
      <table class="table table-zebra w-full">
        ...
        <tbody>
          ...
          <tr v-for="data in store.pagedata" :key="data.id" class="hover">
            ...
            <td class="font-medium">{{ data.url || '-' }}</td>
            <td class="font-medium">{{ data.parentId || '-' }}</td>
            <td class="font-medium">{{ data.bigIcon || '-' }}</td>
            <td class="font-medium">{{ data.smallIcon || '-' }}</td>
            <td>
    ...
    <!-- Details View Modal -->
    <dialog id="m_menu_detail_modal" class="modal">
      <div class="modal-box max-w-md">
        <h3 class="font-bold text-xl mb-4">M Menu Details</h3>
        ...
        <div v-if="selectedItem" class="flex flex-col items-center space-y-4">
          ...
          <div class="w-full bg-base-200 rounded-box p-4 space-y-3">
            ...
            <div class="flex justify-between border-b border-base-300 pb-2">
              <span class="text-sm font-medium opacity-70">Url</span>
              <span class="font-semibold">{{ selectedItem.url ?? '-' }}</span>
            </div>
            <div class="flex justify-between border-b border-base-300 pb-2">
              <span class="text-sm font-medium opacity-70">Parent Id</span>
              <span class="font-semibold">{{ selectedItem.parentId ?? '-' }}</span>
            </div>
            <div class="flex justify-between border-b border-base-300 pb-2">
              <span class="text-sm font-medium opacity-70">Big Icon</span>
              <span class="font-semibold">{{ selectedItem.bigIcon ?? '-' }}</span>
            </div>
            <div class="flex justify-between border-b border-base-300 pb-2">
              <span class="text-sm font-medium opacity-70">Small Icon</span>
              <span class="font-semibold">{{ selectedItem.smallIcon ?? '-' }}</span>
            </div>
          </div>
        </div>
    ...
    <dialog id="m_menu_modal" class="modal">
      <div class="modal-box">
        <h3 class="font-bold text-lg">{ isEdit ? 'Edit' : 'Create' } M Menu</h3>
        ...
        <div class="py-4 space-y-4">
          ...
          <div class="form-control">
            ...
            <input v-model="store.data.url" type="text" class="input input-bordered w-full" />
          </div>

          <div class="form-control">
            <label class="label"><span class="label-text">Parent Id</span></label>
            <input v-model.number="store.data.parentId" type="number" class="input input-bordered w-full" />
          </div>

          <div class="form-control">
            <label class="label"><span class="label-text">Big Icon</span></label>
            <input v-model="store.data.bigIcon" type="text" class="input input-bordered w-full" />
          </div>

          <div class="form-control">
            <label class="label"><span class="label-text">Small Icon</span></label>
            <input v-model="store.data.smallIcon" type="text" class="input input-bordered w-full" />
          </div>
        </div>
