...
import { useTDoctorOfficeScheduleStore } from '@/modules/hospital/t-doctor-office-schedule/store';
import { computed, onMounted, ref } from 'vue';

const store = useTDoctorOfficeScheduleStore();
...
const openDetailModal = (item: any) => {
  ...
  (document.getElementById('t_doctor_office_schedule_detail_modal') as HTMLDialogElement).showModal();
};
...
const openModal = (item: any = null) => {
  ...
  if (item) {
    ...
    store.data = { doctorId: null, medicalFacilityScheduleId: null, slot: null };
  }
  (document.getElementById('t_doctor_office_schedule_modal') as HTMLDialogElement).showModal();
};

const submitForm = async () => {
  await store.saveData(selectedId.value);
  (document.getElementById('t_doctor_office_schedule_modal') as HTMLDialogElement).close();
};
...
<template>
  <div class="p-6">
    <div class="flex justify-between items-center mb-4">
      <h1 class="text-2xl font-bold">T Doctor Office Schedule Management</h1>
      ...
      <table class="table table-zebra w-full">
        <thead>
          <tr class="bg-base-200">
            <th>ID</th>
            <th>Doctor Id</th>
            <th>Medical Facility Schedule Id</th>
            <th>Slot</th>
        ...
        <tbody>
          ...
          <tr v-for="data in store.pagedata" :key="data.id" class="hover">
            ...
            <td class="font-medium">{{ data.medicalFacilityScheduleId || '-' }}</td>
            <td class="font-medium">{{ data.slot || '-' }}</td>
            <td>
    ...
    <!-- Details View Modal -->
    <dialog id="t_doctor_office_schedule_detail_modal" class="modal">
      <div class="modal-box max-w-md">
        <h3 class="font-bold text-xl mb-4">T Doctor Office Schedule Details</h3>
        ...
        <div v-if="selectedItem" class="flex flex-col items-center space-y-4">
          ...
          <div class="w-full bg-base-200 rounded-box p-4 space-y-3">
            ...
            <div class="flex justify-between border-b border-base-300 pb-2">
              <span class="text-sm font-medium opacity-70">Medical Facility Schedule Id</span>
              <span class="font-semibold">{{ selectedItem.medicalFacilityScheduleId ?? '-' }}</span>
            </div>
            <div class="flex justify-between border-b border-base-300 pb-2">
              <span class="text-sm font-medium opacity-70">Slot</span>
              <span class="font-semibold">{{ selectedItem.slot ?? '-' }}</span>
            </div>
          </div>
        </div>
    ...
    <dialog id="t_doctor_office_schedule_modal" class="modal">
      <div class="modal-box">
        <h3 class="font-bold text-lg">{{ isEdit ? 'Edit' : 'Create' }} T Doctor Office Schedule</h3>
        ...
        <div class="py-4 space-y-4">
          ...
          <div class="form-control">
            <label class="label"><span class="label-text">Medical Facility Schedule Id</span></label>
            <input v-model.number="store.data.medicalFacilityScheduleId" type="number" class="input input-bordered w-full" />
          </div>

          <div class="form-control">
            <label class="label"><span class="label-text">Slot</span></label>
            <input v-model.number="store.data.slot" type="number" class="input input-bordered w-full" />
          </div>
        </div>
