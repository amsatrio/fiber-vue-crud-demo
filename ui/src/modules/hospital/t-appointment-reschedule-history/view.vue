...
import { useTAppointmentRescheduleHistoryStore } from '@/modules/hospital/t-appointment-reschedule-history/store';
import { computed, onMounted, ref } from 'vue';

const store = useTAppointmentRescheduleHistoryStore();
...
const openDetailModal = (item: any) => {
  ...
  (document.getElementById('t_appointment_reschedule_history_detail_modal') as HTMLDialogElement).showModal();
};
...
const openModal = (item: any = null) => {
  ...
  if (item) {
    ...
    store.data = { appointmentId: null, doctorOfficeScheduleId: null, doctorOfficeTreatmentId: null, appointmentDate: '' };
  }
  (document.getElementById('t_appointment_reschedule_history_modal') as HTMLDialogElement).showModal();
};

const submitForm = async () => {
  await store.saveData(selectedId.value);
  (document.getElementById('t_appointment_reschedule_history_modal') as HTMLDialogElement).close();
};
...
<template>
  <div class="p-6">
    <div class="flex justify-between items-center mb-4">
      <h1 class="text-2xl font-bold">T Appointment Reschedule History Management</h1>
      ...
      <table class="table table-zebra w-full">
        <thead>
          <tr class="bg-base-200">
            ...
            <th>Doctor Office Schedule Id</th>
        ...
        <tbody>
          ...
          <tr v-for="data in store.pagedata" :key="data.id" class="hover">
            <th>{{ data.id }}</th>
            <td class="font-medium">{{ data.appointmentId || '-' }}</td>
            <td class="font-medium">{{ data.doctorOfficeScheduleId || '-' }}</td>
            <td class="font-medium">{{ data.doctorOfficeTreatmentId || '-' }}</td>
            <td class="font-medium">{{ data.appointmentDate || '-' }}</td>
            <td>
    ...
    <!-- Details View Modal -->
    <dialog id="t_appointment_reschedule_history_detail_modal" class="modal">
      <div class="modal-box max-w-md">
        <h3 class="font-bold text-xl mb-4">T Appointment Reschedule History Details</h3>
        ...
        <div v-if="selectedItem" class="flex flex-col items-center space-y-4">
          ...
          <div class="w-full bg-base-200 rounded-box p-4 space-y-3">
            ...
            <div class="flex justify-between border-b border-base-300 pb-2">
              <span class="text-sm font-medium opacity-70">Appointment Id</span>
              <span class="font-semibold">{{ selectedItem.appointmentId ?? '-' }}</span>
            </div>
            <div class="flex justify-between border-b border-base-300 pb-2">
              <span class="text-sm font-medium opacity-70">Doctor Office Schedule Id</span>
              <span class="font-semibold">{{ selectedItem.doctorOfficeScheduleId ?? '-' }}</span>
            </div>
            <div class="flex justify-between border-b border-base-300 pb-2">
            ...
            <div class="flex justify-between border-b border-base-300 pb-2">
              <span class="text-sm font-medium opacity-70">Appointment Date</span>
              <span class="font-semibold">{{ selectedItem.appointmentDate ?? '-' }}</span>
            </div>
          </div>
        </div>
    ...
    <dialog id="t_appointment_reschedule_history_modal" class="modal">
      <div class="modal-box">
        <h3 class="font-bold text-lg">{{ isEdit ? 'Edit' : 'Create' }} T Appointment Reschedule History</h3>
        
        <div class="py-4 space-y-4">
          <div class="form-control">
            <label class="label"><span class="label-text">Appointment Id</span></label>
            <input v-model.number="store.data.appointmentId" type="number" class="input input-bordered w-full" />
          </div>

          <div class="form-control">
            <label class="label"><span class="label-text">Doctor Office Schedule Id</span></label>
            <input v-model.number="store.data.doctorOfficeScheduleId" type="number" class="input input-bordered w-full" />
          </div>
          ...
          <div class="form-control">
            <label class="label"><span class="label-text">Appointment Date</span></label>
            <input v-model="store.data.appointmentDate" type="date" class="input input-bordered w-full" />
          </div>
        </div>
