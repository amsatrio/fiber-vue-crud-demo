...
import { useMMedicalFacilityScheduleStore } from '@/modules/hospital/m-medical-facility-schedule/store';
import { computed, onMounted, ref } from 'vue';

const store = useMMedicalFacilityScheduleStore();
...
const openDetailModal = (item: any) => {
  ...
  (document.getElementById('m_medical_facility_schedule_detail_modal') as HTMLDialogElement).showModal();
};
...
const openModal = (item: any = null) => {
  ...
  if (item) {
    ...
    store.data = { medicalFacilityId: null, day: '', timeScheduleStart: '', timeScheduleEnd: '' };
  }
  (document.getElementById('m_medical_facility_schedule_modal') as HTMLDialogElement).showModal();
};

const submitForm = async () => {
  await store.saveData(selectedId.value);
  (document.getElementById('m_medical_facility_schedule_modal') as HTMLDialogElement).close();
};
...
<template>
  <div class="p-6">
    <div class="flex justify-between items-center mb-4">
      <h1 class="text-2xl font-bold">M Medical Facility Schedule Management</h1>
      ...
      <table class="table table-zebra w-full">
        <thead>
          <tr class="bg-base-200">
            ...
            <th>Day</th>
            <th>Time Schedule Start</th>
            <th>Time Schedule End</th>
        ...
        <tbody>
          ...
          <tr v-for="data in store.pagedata" :key="data.id" class="hover">
            ...
            <td class="font-medium">{{ data.timeScheduleStart || '-' }}</td>
            <td class="font-medium">{{ data.timeScheduleEnd || '-' }}</td>
            <td>
    ...
    <!-- Details View Modal -->
    <dialog id="m_medical_facility_schedule_detail_modal" class="modal">
      <div class="modal-box max-w-md">
        <h3 class="font-bold text-xl mb-4">M Medical Facility Schedule Details</h3>
        ...
        <div v-if="selectedItem" class="flex flex-col items-center space-y-4">
          ...
          <div class="w-full bg-base-200 rounded-box p-4 space-y-3">
            ...
            <div class="flex justify-between border-b border-base-300 pb-2">
              <span class="text-sm font-medium opacity-70">Day</span>
              <span class="font-semibold">{{ selectedItem.day ?? '-' }}</span>
            </div>
            <div class="flex justify-between border-b border-base-300 pb-2">
              <span class="text-sm font-medium opacity-70">Time Schedule Start</span>
              <span class="font-semibold">{{ selectedItem.timeScheduleStart ?? '-' }}</span>
            </div>
            <div class="flex justify-between border-b border-base-300 pb-2">
              <span class="text-sm font-medium opacity-70">Time Schedule End</span>
              <span class="font-semibold">{{ selectedItem.timeScheduleEnd ?? '-' }}</span>
            </div>
          </div>
        </div>
    ...
    <dialog id="m_medical_facility_schedule_modal" class="modal">
      <div class="modal-box">
        <h3 class="font-bold text-lg">{{ isEdit ? 'Edit' : 'Create' }} M Medical Facility Schedule</h3>
        ...
        <div class="py-4 space-y-4">
          ...
          <div class="form-control">
            ...
            <input v-model="store.data.day" type="text" class="input input-bordered w-full" />
          </div>

          <div class="form-control">
            <label class="label"><span class="label-text">Time Schedule Start</span></label>
            <input v-model="store.data.timeScheduleStart" type="text" class="input input-bordered w-full" />
          </div>

          <div class="form-control">
            <label class="label"><span class="label-text">Time Schedule End</span></label>
            <input v-model="store.data.timeScheduleEnd" type="text" class="input input-bordered w-full" />
          </div>
        </div>
