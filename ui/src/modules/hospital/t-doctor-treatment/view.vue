...
import { useTDoctorTreatmentStore } from '@/modules/hospital/t-doctor-treatment/store';
import { computed, onMounted, ref } from 'vue';

const store = useTDoctorTreatmentStore();
...
const openDetailModal = (item: any) => {
  ...
  (document.getElementById('t_doctor_treatment_detail_modal') as HTMLDialogElement).showModal();
};
...
const openModal = (item: any = null) => {
  ...
  if (item) {
    ...
    store.data = { doctorId: null, name: '' };
  }
  (document.getElementById('t_doctor_treatment_modal') as HTMLDialogElement).showModal();
};

const submitForm = async () => {
  await store.saveData(selectedId.value);
  (document.getElementById('t_doctor_treatment_modal') as HTMLDialogElement).close();
};
...
<template>
  <div class="p-6">
    <div class="flex justify-between items-center mb-4">
      <h1 class="text-2xl font-bold">T Doctor Treatment Management</h1>
    ...
    <!-- Details View Modal -->
    <dialog id="t_doctor_treatment_detail_modal" class="modal">
      <div class="modal-box max-w-md">
        <h3 class="font-bold text-xl mb-4">T Doctor Treatment Details</h3>
    ...
    <dialog id="t_doctor_treatment_modal" class="modal">
      <div class="modal-box">
        <h3 class="font-bold text-lg">{{ isEdit ? 'Edit' : 'Create' }} T Doctor Treatment</h3>
