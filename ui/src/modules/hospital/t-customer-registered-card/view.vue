...
import { useTCustomerRegisteredCardStore } from '@/modules/hospital/t-customer-registered-card/store';
import { computed, onMounted, ref } from 'vue';

const store = useTCustomerRegisteredCardStore();
...
const openDetailModal = (item: any) => {
  ...
  (document.getElementById('t_customer_registered_card_detail_modal') as HTMLDialogElement).showModal();
};
...
const openModal = (item: any = null) => {
  ...
  if (item) {
    ...
    store.data = { customerId: null, cardNumber: '', validityPeriod: '', cvv: '' };
  }
  (document.getElementById('t_customer_registered_card_modal') as HTMLDialogElement).showModal();
};

const submitForm = async () => {
  await store.saveData(selectedId.value);
  (document.getElementById('t_customer_registered_card_modal') as HTMLDialogElement).close();
};
...
<template>
  <div class="p-6">
    <div class="flex justify-between items-center mb-4">
      <h1 class="text-2xl font-bold">T Customer Registered Card Management</h1>
      ...
      <table class="table table-zebra w-full">
        ...
        <tbody>
          ...
          <tr v-for="data in store.pagedata" :key="data.id" class="hover">
            ...
            <td class="font-medium">{{ data.cardNumber || '-' }}</td>
            <td class="font-medium">{{ data.validityPeriod || '-' }}</td>
            <td class="font-medium">{{ data.cvv || '-' }}</td>
            <td>
    ...
    <!-- Details View Modal -->
    <dialog id="t_customer_registered_card_detail_modal" class="modal">
      <div class="modal-box max-w-md">
        <h3 class="font-bold text-xl mb-4">T Customer Registered Card Details</h3>
        ...
        <div v-if="selectedItem" class="flex flex-col items-center space-y-4">
          ...
          <div class="w-full bg-base-200 rounded-box p-4 space-y-3">
            ...
            <div class="flex justify-between border-b border-base-300 pb-2">
              <span class="text-sm font-medium opacity-70">Card Number</span>
              <span class="font-semibold">{{ selectedItem.cardNumber ?? '-' }}</span>
            </div>
            <div class="flex justify-between border-b border-base-300 pb-2">
              <span class="text-sm font-medium opacity-70">Validity Period</span>
              <span class="font-semibold">{{ selectedItem.validityPeriod ?? '-' }}</span>
            </div>
            <div class="flex justify-between border-b border-base-300 pb-2">
              <span class="text-sm font-medium opacity-70">Cvv</span>
              <span class="font-semibold">{{ selectedItem.cvv ?? '-' }}</span>
            </div>
          </div>
        </div>
    ...
    <dialog id="t_customer_registered_card_modal" class="modal">
      <div class="modal-box">
        <h3 class="font-bold text-lg">{{ isEdit ? 'Edit' : 'Create' }} T Customer Registered Card</h3>
        ...
        <div class="py-4 space-y-4">
          ...
          <div class="form-control">
            ...
            <input v-model="store.data.cardNumber" type="text" class="input input-bordered w-full" />
          </div>

          <div class="form-control">
            <label class="label"><span class="label-text">Validity Period</span></label>
            <input v-model="store.data.validityPeriod" type="date" class="input input-bordered w-full" />
          </div>

          <div class="form-control">
            <label class="label"><span class="label-text">Cvv</span></label>
            <input v-model="store.data.cvv" type="text" class="input input-bordered w-full" />
          </div>
        </div>
