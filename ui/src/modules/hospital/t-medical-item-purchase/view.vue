...
import { useTMedicalItemPurchaseStore } from '@/modules/hospital/t-medical-item-purchase/store';
import { computed, onMounted, ref } from 'vue';

const store = useTMedicalItemPurchaseStore();
...
const openModal = (item: any = null) => {
  ...
  if (item) {
    ...
    store.data = { customerId: null, paymentMethodId: null };
  }
  (document.getElementById('t_medical_item_purchase_modal') as HTMLDialogElement).showModal();
};

const submitForm = async () => {
  await store.saveData(selectedId.value);
  (document.getElementById('t_medical_item_purchase_modal') as HTMLDialogElement).close();
};
...
<template>
  <div class="p-6">
    <div class="flex justify-between items-center mb-4">
      <h1 class="text-2xl font-bold">T Medical Item Purchase Management</h1>
      ...
      <table class="table table-zebra w-full">
        ...
        <tbody>
          ...
          <tr v-for="data in store.pagedata" :key="data.id" class="hover">
            ...
            <td class="font-medium">{{ data.paymentMethodId || '-' }}</td>
            <td>
    ...
    <dialog id="t_medical_item_purchase_detail_modal" class="modal">
      <div class="modal-box max-w-md">
        <h3 class="font-bold text-xl mb-4">T Medical Item Purchase Details</h3>
        ...
        <div v-if="selectedItem" class="flex flex-col items-center space-y-4">
          ...
          <div class="w-full bg-base-200 rounded-box p-4 space-y-3">
            ...
            <div class="flex justify-between border-b border-base-300 pb-2">
              <span class="text-sm font-medium opacity-70">Payment Method Id</span>
              <span class="font-semibold">{{ selectedItem.paymentMethodId ?? '-' }}</span>
            </div>
          </div>
        </div>
    ...
    <dialog id="t_medical_item_purchase_modal" class="modal">
      <div class="modal-box">
        <h3 class="font-bold text-lg">{ isEdit ? 'Edit' : 'Create' } T Medical Item Purchase</h3>
        ...
        <div class="py-4 space-y-4">
          ...
          <div class="form-control">
            <label class="label"><span class="label-text">Payment Method Id</span></label>
            <input v-model.number="store.data.paymentMethodId" type="number" class="input input-bordered w-full" />
          </div>
        </div>
