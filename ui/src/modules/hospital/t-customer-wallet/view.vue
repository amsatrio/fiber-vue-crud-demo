...
import { useTCustomerWalletStore } from '@/modules/hospital/t-customer-wallet/store';
import { computed, onMounted, ref } from 'vue';

const store = useTCustomerWalletStore();
...
const openDetailModal = (item: any) => {
  ...
  (document.getElementById('t_customer_wallet_detail_modal') as HTMLDialogElement).showModal();
};
...
const openModal = (item: any = null) => {
  ...
  if (item) {
    ...
    store.data = { customerId: null, pin: '', balance: null, barcode: '', points: null };
  }
  (document.getElementById('t_customer_wallet_modal') as HTMLDialogElement).showModal();
};

const submitForm = async () => {
  await store.saveData(selectedId.value);
  (document.getElementById('t_customer_wallet_modal') as HTMLDialogElement).close();
};
...
<template>
  <div class="p-6">
    <div class="flex justify-between items-center mb-4">
      <h1 class="text-2xl font-bold">T Customer Wallet Management</h1>
      ...
      <table class="table table-zebra w-full">
        ...
        <tbody>
          ...
          <tr v-for="data in store.pagedata" :key="data.id" class="hover">
            ...
            <td class="font-medium">{{ data.pin || '-' }}</td>
            <td class="font-medium">{{ data.balance || '-' }}</td>
            <td class="font-medium">{{ data.barcode || '-' }}</td>
            <td class="font-medium">{{ data.points || '-' }}</td>
            <td>
    ...
    <!-- Details View Modal -->
    <dialog id="t_customer_wallet_detail_modal" class="modal">
      <div class="modal-box max-w-md">
        <h3 class="font-bold text-xl mb-4">T Customer Wallet Details</h3>
        ...
        <div v-if="selectedItem" class="flex flex-col items-center space-y-4">
          ...
          <div class="w-full bg-base-200 rounded-box p-4 space-y-3">
            ...
            <div class="flex justify-between border-b border-base-300 pb-2">
              <span class="text-sm font-medium opacity-70">Pin</span>
              <span class="font-semibold">{{ selectedItem.pin ?? '-' }}</span>
            </div>
            <div class="flex justify-between border-b border-base-300 pb-2">
              <span class="text-sm font-medium opacity-70">Balance</span>
              <span class="font-semibold">{{ selectedItem.balance ?? '-' }}</span>
            </div>
            <div class="flex justify-between border-b border-base-300 pb-2">
              <span class="text-sm font-medium opacity-70">Barcode</span>
              <span class="font-semibold">{{ selectedItem.barcode ?? '-' }}</span>
            </div>
            <div class="flex justify-between border-b border-base-300 pb-2">
              <span class="text-sm font-medium opacity-70">Points</span>
              <span class="font-semibold">{{ selectedItem.points ?? '-' }}</span>
            </div>
          </div>
        </div>
    ...
    <dialog id="t_customer_wallet_modal" class="modal">
      <div class="modal-box">
        <h3 class="font-bold text-lg">{ isEdit ? 'Edit' : 'Create' } T Customer Wallet</h3>
        ...
        <div class="py-4 space-y-4">
          ...
          <div class="form-control">
            ...
            <input v-model="store.data.pin" type="text" class="input input-bordered w-full" />
          </div>

          <div class="form-control">
            <label class="label"><span class="label-text">Balance</span></label>
            <input v-model.number="store.data.balance" type="number" class="input input-bordered w-full" />
          </div>

          <div class="form-control">
            <label class="label"><span class="label-text">Barcode</span></label>
            <input v-model="store.data.barcode" type="text" class="input input-bordered w-full" />
          </div>

          <div class="form-control">
            <label class="label"><span class="label-text">Points</span></label>
            <input v-model.number="store.data.points" type="number" class="input input-bordered w-full" />
          </div>
        </div>
