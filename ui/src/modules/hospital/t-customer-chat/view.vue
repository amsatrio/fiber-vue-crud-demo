...
import { useTCustomerChatStore } from '@/modules/hospital/t-customer-chat/store';
import { computed, onMounted, ref } from 'vue';

const store = useTCustomerChatStore();
...
const openDetailModal = (item: any) => {
  ...
  (document.getElementById('t_customer_chat_detail_modal') as HTMLDialogElement).showModal();
};
...
const openModal = (item: any = null) => {
  ...
  if (item) {
    ...
    store.data = { customerId: null, doctorId: null };
  }
  (document.getElementById('t_customer_chat_modal') as HTMLDialogElement).showModal();
};

const submitForm = async () => {
  await store.saveData(selectedId.value);
  (document.getElementById('t_customer_chat_modal') as HTMLDialogElement).close();
};
...
<template>
  <div class="p-6">
    <div class="flex justify-between items-center mb-4">
      <h1 class="text-2xl font-bold">T Customer Chat Management</h1>
    ...
    <!-- Details View Modal -->
    <dialog id="t_customer_chat_detail_modal" class="modal">
      <div class="modal-box max-w-md">
        <h3 class="font-bold text-xl mb-4">T Customer Chat Details</h3>
    ...
    <dialog id="t_customer_chat_modal" class="modal">
      <div class="modal-box">
        <h3 class="font-bold text-lg">{ isEdit ? 'Edit' : 'Create' } T Customer Chat</h3>
