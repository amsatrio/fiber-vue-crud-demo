...
import { useMBiodataAttachmentStore } from '@/modules/hospital/m-biodata-attachment/store';
import { computed, onMounted, ref } from 'vue';

const store = useMBiodataAttachmentStore();
...
const openDetailModal = (item: any) => {
  ...
  (document.getElementById('m_biodata_attachment_detail_modal') as HTMLDialogElement).showModal();
};
...
const openModal = (item: any = null) => {
  ...
  if (item) {
    ...
    store.data = { biodataId: null, fileName: '', filePath: '', fileSize: null, file: null };
  }
  (document.getElementById('m_biodata_attachment_modal') as HTMLDialogElement).showModal();
};

const submitForm = async () => {
  await store.saveData(selectedId.value);
  (document.getElementById('m_biodata_attachment_modal') as HTMLDialogElement).close();
};
...
<template>
  <div class="p-6">
    <div class="flex justify-between items-center mb-4">
      <h1 class="text-2xl font-bold">M Biodata Attachment Management</h1>
            ...
            <td>
              <div class="avatar">
                <div class="w-12 h-12 rounded-full border overflow-hidden">
                  <img :src="'data:image/png;base64,' + data.file" alt="Avatar" class="object-cover w-full h-full" />
                </div>
              </div>
            </td>
            <td class="font-medium">{{ data.biodataId || '-' }}</td>
            <td class="font-medium">{{ data.fileName || '-' }}</td>
            <td class="font-medium">{{ data.filePath || '-' }}</td>
            <td class="font-medium">{{ data.fileSize || '-' }}</td>
            <td>
    ...
    <!-- Details View Modal -->
    <dialog id="m_biodata_attachment_detail_modal" class="modal">
      <div class="modal-box max-w-md">
        <h3 class="font-bold text-xl mb-4">M Biodata Attachment Details</h3>
        ...
        <div v-if="selectedItem" class="flex flex-col items-center space-y-4">
          ...
          <div class="avatar">
            <div class="w-36 h-36 rounded-2xl ring ring-primary ring-offset-base-100 ring-offset-2 overflow-hidden shadow-md">
              <img 
                :src="'data:image/png;base64,' + selectedItem.file" 
                :alt="selectedItem.id" 
          ...
          <div class="w-full bg-base-200 rounded-box p-4 space-y-3">
            ...
            <div class="flex justify-between border-b border-base-300 pb-2">
              <span class="text-sm font-medium opacity-70">File Name</span>
              <span class="font-semibold">{{ selectedItem.fileName ?? '-' }}</span>
            </div>
            <div class="flex justify-between border-b border-base-300 pb-2">
              <span class="text-sm font-medium opacity-70">File Path</span>
              <span class="font-semibold">{{ selectedItem.filePath ?? '-' }}</span>
            </div>
            <div class="flex justify-between border-b border-base-300 pb-2">
              <span class="text-sm font-medium opacity-70">File Size</span>
              <span class="font-semibold">{{ selectedItem.fileSize ?? '-' }}</span>
            </div>
          </div>
        </div>
    ...
    <dialog id="m_biodata_attachment_modal" class="modal">
      <div class="modal-box">
        <h3 class="font-bold text-lg">{ isEdit ? 'Edit' : 'Create' } M Biodata Attachment</h3>
        ...
        <div class="py-4 space-y-4">
          ...
          <div class="form-control">
            ...
            <input v-model="store.data.fileName" type="text" class="input input-bordered w-full" />
          </div>

          <div class="form-control">
            <label class="label"><span class="label-text">File Path</span></label>
            <input v-model="store.data.filePath" type="text" class="input input-bordered w-full" />
          </div>

          <div class="form-control">
            <label class="label"><span class="label-text">File Size</span></label>
            <input v-model.number="store.data.fileSize" type="number" class="input input-bordered w-full" />
          </div>
